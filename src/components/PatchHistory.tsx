...
export const PatchHistory: Component<{ item: UnifiedItem; civ?: civConfig }> = (props) => {
  const [patchHistory] = createResource(() => getPatchHistory(props.item, props.civ ? [props.civ] : undefined), { initialValue: [] });
  const [patchesExpanded, setPatchesExpanded] = createSignal(false);
  return (
    <Show when={!!patchHistory().length && patchHistory()} keyed>
      {(fullHistory) =>
        <div class="my-8">
          <h3 class="text-lg text-white font-bold mb-4">Patch History</h3>
          <For each={fullHistory.slice(0, patchesExpanded() ? undefined : 2)}>
            {(history) => {
                    ...
                    {([type, change, civs]) => (
                      <div class="flex  py-0.5">
                        ...
                        <p class="text-gray-100 text-base max-w-prose white">
                          {!props.civ && (civs || []).map((c) => <CivFlag abbr={c} class="h-3 rounded-sm mr-1.5 mb-0.5 inline" />)}
                          {change}
                        </p>
                      </div>
                    )}
          ...
          <Show when={fullHistory.length > 2 && !patchesExpanded()}>
