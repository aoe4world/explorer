...
import { CIVILIZATIONS, CivConfig } from "../../config";
import { TechnologySelector } from "./TechnologySelector";
import { getItemHref } from "@components/Cards";
import { A } from "@solidjs/router";
import { BattleStats, getBattleStats } from "../../query/battlereport";

function formatExplosiveMessage(stats1?: BattleStats, stats2?: BattleStats) {
...
export const BattleReportView: Component<{ unit1: UnifiedItem<Unit>; unit2: UnifiedItem<Unit> | undefined; age1?: number; age2?: number; variation1: Unit; variation2: Unit | undefined, onCivSelect?: (civ: CivConfig, unitNumber: 1 | 2) => void }> = (props) => {
  ...
  const Unit = (props: { unit: string; }) => <span class="text-sm text-gray-300 text-center pl-1">{props.unit}</span>;

  const formatNumber = (value?: string | number, fixed?: number) => {
  ...
  const StatLine = (props: {
    sizeClass?: string;
    class?: string,
    leftStats?: BattleStats,
    rightStats?: BattleStats,
    styleFunc?: (stat?: BattleStats, other?: BattleStats) => string | null;
    valueFunc: (stat?: BattleStats, other?: BattleStats) => string | number | undefined;
    fixed?: number,
    unit?: string
  }) => {
    return <>
      ...
      <div class={`col-start-3 ${props.sizeClass ?? "text-xl"} ${props.class} ${props.styleFunc ? props.styleFunc(props.rightStats, props.leftStats) : ''}`}>{formatNumber(props.valueFunc(props.leftStats, props.rightStats), props.fixed)}{props.unit && <Unit unit={props.unit}/>}</div>
    </>;
  };

  const battleReportHref = () => props.variation1 && props.variation2 && getItemHref(props.unit1, CIVILIZATIONS[props.variation1.civs[0]]) + "/versus/" + CIVILIZATIONS[props.variation2.civs[0]].slug + "/units/" + props.unit2!.id + `?age=${props.variation1.age}&targetAge=${props.variation2.age}`;

  return (
    <div class="flex flex-col gap-4">
      <div class="grid grid-cols-[3fr_2fr_3fr] gap-4">
        <UnitCard unit={props.unit1} age={props.age1} variation={props.variation1} civ={CIVILIZATIONS[props.variation1.civs[0]]} selectedTechnologies={selectedTechs1()} linkType="blank" onCivSelect={(civ) => props.onCivSelect?.(civ, 1)} />

        <div class="grid grid-cols-[auto_1fr_auto] text-md mb-auto">
          <h4 class="col-span-3 font-bold text-white text-xl text-center pb-10">
            <Show when={battleReportHref()} fallback={<>Battle Report</>}>
              {(href) => <A href={href()} target="about:blank">Battle Report</A>}
            </Show>
          </h4>
          ...
          <StatLine leftStats={stats1()} rightStats={stats2()} styleFunc={(stat, other) => (stat?.attacksRequired ?? 0) < (other?.attacksRequired ?? 0) ? winnerClass : drawClass} valueFunc={(stat) => stat?.attacksRequired} unit="attacks" />
          ...
          <StatLine leftStats={stats1()} rightStats={stats2()} styleFunc={(stat, other) => (stat?.timeRequired ?? 0) < (other?.timeRequired ?? 0) ? winnerClass : drawClass} valueFunc={(stat) => stat?.timeRequired} fixed={1} unit="sec" />
        ...
        <Show when={props.unit2 && props.variation2} fallback={<div class="bg-item-unit rounded-2xl bg-opacity-10 hover:bg-opacity-20"/>}>
          <UnitCard unit={props.unit2!} age={props.age2} variation={props.variation2} civ={CIVILIZATIONS[props.variation2!.civs[0]]} selectedTechnologies={selectedTechs2()} linkType="blank" onCivSelect={(civ) => props.onCivSelect?.(civ, 2)} />
        </Show>

        <div>
          <TechnologySelector item={props.unit1} civ={CIVILIZATIONS[props.variation1.civs[0]]} age={props.age1} onChange={setSelectedTechs1} />
        </div>
        <div class="col-start-3">
          <Show when={props.unit2 && props.variation2}>
            <TechnologySelector item={props.unit2!} civ={CIVILIZATIONS[props.variation2!.civs[0]]} age={props.age2} onChange={setSelectedTechs2} />
          </Show>
        </div>
      </div>
    </div>
